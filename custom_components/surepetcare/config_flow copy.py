...
class SurePetCareConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_configure_devices(
        ...
        for device in self.devices:
            ...
            description_placeholders[f"{device.id}"] = device.name

            # section_title = self.hass.config_entries.async_entry_for_domain_unique_idtranslation.async_translate(
            #    "config.step.configure_devices.section_title",
            #    {"device_name": device.name, "product_id": device.product_id},
            # )
            schema_dict = section(
                vol.Schema(schema_info["schema"]),
                SectionConfig(translation_key="section_title", collapsed=True),
            )
        # await async_get_translations(self.hass, "en", "", None, config_flow=True)
