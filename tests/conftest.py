...
class DummyEntry:
    ...
    subentries = {"subid": MagicMock(data={"product_id": "DUAL_SCAN_PET_DOOR", "name": "TestDevice", "location_inside": "A", "location_outside": "B"})}
...
def make_dummy_handler(handler_cls: type, entry=None, subentry=None, source: str = "user"):
...
class DummyClient:
    ...
    async def api(self, arg=None):
        ...
        if arg == DummyHousehold.get_households():
            ...
            return []
        return "data"
    async def login(self, token=None, device_id=None, email=None, password=None, **kwargs) -> bool:
...
async def dummy_success_client(monkeypatch) -> DummyClient:
    ...
    class SuccessClient(DummyClient):
        ...
        async def close(self):
            pass
    monkeypatch.setattr("custom_components.surepetcare.config_flow.SurePetcareClient", SuccessClient)
