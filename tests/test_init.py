...
import pytest
from tests.conftest import DummyHass, DummyConfigEntry, DummyClient, DummyHousehold, FakeDevice, FailingClient, ExceptionClient, async_forward_entry_setups
...
async def test_async_setup_entry_and_unload():
    ...
    with patch("custom_components.surepetcare.__init__.SurePetcareClient", DummyClient), \
         patch("custom_components.surepetcare.__init__.Household", DummyHousehold), \
         patch("homeassistant.helpers.device_registry.async_get", lambda hass: MagicMock()):
        if hasattr(surepetcare_init, "remove_stale_devices"):
            with patch("custom_components.surepetcare.__init__.remove_stale_devices", lambda *a, **kw: None):
...
async def test_async_setup_entry_login_failure():
    ...
    entry = DummyConfigEntry()
    hass.config_entries.async_forward_entry_setups = async_forward_entry_setups
    with patch("custom_components.surepetcare.__init__.SurePetcareClient", FailingClient), \
         patch("custom_components.surepetcare.__init__.Household", DummyHousehold), \
         patch("homeassistant.helpers.device_registry.async_get", lambda hass: MagicMock()):
        try:
            await surepetcare_init.async_setup_entry(hass, entry)
...
async def test_async_setup_entry_api_exception():
    ...
    entry = DummyConfigEntry()
    hass.config_entries.async_forward_entry_setups = async_forward_entry_setups
    with patch("custom_components.surepetcare.__init__.SurePetcareClient", ExceptionClient), \
         patch("custom_components.surepetcare.__init__.Household", DummyHousehold), \
         patch("homeassistant.helpers.device_registry.async_get", lambda hass: MagicMock()):
        try:
            await surepetcare_init.async_setup_entry(hass, entry)
...
async def test_remove_stale_devices_called():
    ...
    def fake_remove_stale_devices(*a, **kw):
        called["called"] = True
    with patch("custom_components.surepetcare.__init__.remove_stale_devices", fake_remove_stale_devices), \
         patch("custom_components.surepetcare.__init__.SurePetcareClient", DummyClient), \
         patch("custom_components.surepetcare.__init__.Household", DummyHousehold), \
         patch("homeassistant.helpers.device_registry.async_get", lambda hass: MagicMock()):
        await surepetcare_init.async_setup_entry(hass, entry)
...
def test_remove_stale_devices_logic():
    ...
    with patch("custom_components.surepetcare.__init__.dr.async_get", return_value=device_registry), \
         patch("custom_components.surepetcare.__init__.dr.async_entries_for_config_entry", return_value=[matching_entry, stale_entry]):
