...
def test_traverse_path_dict():
    data = {"a": {"b": 42}}
    assert get_by_paths(data, ["a.b"]) == {"a_b": 42}

def test_traverse_path_list():
    data = {"a": [10, 20, 30]}
    assert get_by_paths(data, ["a.1"]) == {"a_1": 20}

def test_traverse_path_dataclass():
    data = Outer(inner=Inner(foo=5, bar="baz"), items=[1, 2], mapping={"x": 7})
    assert get_by_paths(data, ["inner.foo"]) == {"inner_foo": 5}
    assert get_by_paths(data, ["items.1"]) == {"items_1": 2}
    assert get_by_paths(data, ["mapping.x"]) == {"mapping_x": 7}
...
def test_multiple_paths():
    ...
    result = get_by_paths(data, ["a.0", "b.x"])
...
def test_list_of_dataclass_wildcard():
    data = {"bowls": [Bowl(id=1, name="A", weight=10.5), Bowl(id=2, name="B", weight=20.0)]}
    # Should unmap the dataclass at index 0
    result = get_by_paths(data, ["bowls.0.*"])
...
def test_recursive_get_else_branch():
    ...
    from custom_components.surepetcare.entity import get_by_paths
    class Dummy:
        pass
    data = Dummy()
