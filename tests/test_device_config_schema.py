...
def test_dual_scan_pet_door_schema_missing_keys():
    ...
    valid = {"location_inside": "Hall", "location_outside": "Garden", "extra": "ignored"}
    # Should not raise
    assert vol_schema({k: valid[k] for k in ["location_inside", "location_outside"]}) == {"location_inside": "Hall", "location_outside": "Garden"}
...
def test_device_config_schemas_structure():
    ...
    for pid in [ProductId.FEEDER_CONNECT, ProductId.DUAL_SCAN_PET_DOOR, ProductId.HUB, ProductId.PET]:
        assert pid in device_config_schema.DEVICE_CONFIG_SCHEMAS

def test_dual_scan_pet_door_schema_keys():
    schema = device_config_schema.DEVICE_CONFIG_SCHEMAS[ProductId.DUAL_SCAN_PET_DOOR]["schema"]
    assert isinstance(schema, dict)
